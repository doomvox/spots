...
{ my $subname = 'cats';
...

  {
    ...
    my $id     = $cat->id;       # 36
    my $height = $cat->height;   # 1.3 (ok)
    my $width  = $cat->width;    # 60
    my $name   = $cat->name;     # 'politics'
    # say "name: $name  height: $height  width: $width  name: $name";
    is_deeply( [$id, $height, $width, $name], [36, 1.3, 60, 'politics'],
               "$test_name spot check id 36");  # STILL BRITTLE
  }

  {
    ...
    my $id     = $cat->id;      # 57
    my $height = $cat->height;  # 3.9 (ok)
    my $width  = $cat->width;   # 132 (?)
    my $name   = $cat->name;    # 'san_francisco'
    # say "name: $name  height: $height  width: $width  name: $name";
    is_deeply( [$id, $height, $width, $name], [57, 3.9, 132, $exp_name],
               "$test_name spot check idx 5");  # precise but BRITTLE
...
#     }
    ...
    cmp_ok( $length_label, '>=', $l_low,  "$test_name: max label length $length_label > than likely low: $l_low");
    cmp_ok( $length_label, '<=', $l_high, "$test_name: max label length $length_label < than likely high: $l_high");

    # estimating likely range of scaling factor
    my ($scale_low, $scale_high) = (9, 15);
    my ($low, $high) = ($length_label * $scale_low, $length_label * $scale_high);
    cmp_ok( $width, '>=', $low,  "$test_name: width $width > than likely low: $low");
    ...
    # estimating likely range of scaling factor
    my ($y_scale_low, $y_scale_high) = (1, 1.5);
